...
FROM DUAL; -- 9 (INDEX X, 몇 번째에 위치해 있는가 O)
...
-- MONTHS_BETWEEN()은 윤년(2월 29 일이 포함된 해) 계산이 자동으로 수행
--> YEAR, MONTH 단위 계산 시 더 정확한 값을 얻어낼 수 있다!!!

----------------------------------------------------------------

-- ADD_MONTHS(날짜, 숫자) : 날짜를 숫자 만큼의 개월 수를 더하여 반환
...
--EMPLOYEE  테이블애서 2000 년대에 입사한 사원의
...
--화폐기호 - 자릿수 구분
SELECT 
	TO_CHAR(12346789, 'L999999999'),
	TO_CHAR(12346789, '$999999999'),
	TO_CHAR(12346789, '$999,999,999')
FROM DUAL;
...
SELECT EMP_ID, EMP_NAME, 
	NVL2(PHONE , 
	RPAD(SUBSTR(PHONE,1,3), LENGTH(PHONE), '*'), '없음' )
FROM EMPLOYEE ;
...
/*2*/  WHERE DEPT_CODE = 'D6';             --2) DEPT-CODE 가 'D6'인 행만 모아서
...
-- COUNT(DISTINCT 컬럼명) : 
	...
	-- EX) A A B C D D D E : 5개 (중복은 한 번만 카운트)S

--EMPLOYEE 테이블 전체 행의 개수
SELECT COUNT(*)
FROM EMPLOYEE; -- 23행

--EMPLOYEE 테이블에서 부소 코드가 'D5' 인 사원의 수 
SELECT COUNT(*)
FROM EMPLOYEE
