 ...
 * FROM 테이블명;
 * 
 * -> 지정된 테이블에서 모든 행의 컬럼명 이 일치하는 컬럼 값 조회
 * */
...
--SELECT 문 작성 시
--컬럼명에 산술 연산을 직접 작성하면
--조회 결회(RESULT SET)에 연산 결과가 반영되어 조회된다!
...
/*날짜  끼리 연산하기 */
-- 날짜 - 날짜 == 일 단위 숫자
-- 1 == 1일 차이, 1.5 == 1일 12 시간 차이

-- TO_DATE('문자열', '패턴') : 
-- '문자열' 을 '패턴' 형태로 해석해서 DATE 타입으로 변격하는 함수
...
--CEIL(숫자) :올림 처리
-- 현재 시간 - 생년월일
SELECT CEIL ((SYSDATE - TO_DATE('1998-01-14', 'YYYY-MM-DD'))/365)
FROM DUAL;
...
-- 컬럼명 = NULL / 컬럼명 != NULL(X)
	...
	--> DB에서 NULL은 값 x, 저장더ㅣㄴ 값이 없다라는 의미!!!

--컬럼명 IS NULL, 컬럼명 IS NOT NULL(O)
	--> 컬럼 값이 존재하지 않는 경우/ 존재하는 경우


-- EMPLOYEE 태이블에서 DEPT_CODE가 없는 사원 조회
SELECT EMP_NAME, DEPT_CODE
FROM EMPLOYEE 
WHERE DEPT_CODE IS NULL;

-- EMPLOYEE 태이블에서 DEPT_CODE가 있는 사원 조회
SELECT EMP_NAME, DEPT_CODE 
FROM EMPLOYEE 
...
--WHERE PHONE 
--LIKE '010%';  								-- %VERSION
WHERE PHONE LIKE '010________'; -- _VERSION
