...
INSERT INTO "MEMBER" 
VALUES ('MEM03', 'QWERTY', '박철수' , '970303-1234567', DEFAULT);
...
-- NULL을 ("KHK"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
--------------------------------------------------------------------------------------------------------------------
...
CREATE TABLE USER_USED_UK(
    USER_NO NUMBER,
    --USER_ID VARCHAR2(20) UNIQUE, --컬럼 레벨 제악 조건 설정 (이름 X)
    
    --USER_ID VARCHAR2(20) CONSTRAINT USER_ID_U UNIQUE,
    											-- CONSTRAINT 제약조건명 제액조건종류
    											-- 컬럼 레벨 제약 조건 (이름 O)
...
--> USER_NO가 다름
--ORA-00001: 무결성 제약 조건(KHK.USER_ID_NAME_U)에 위배됩니다
...
-- 참조(REFERENCES)된 다른 테이블의 컬럼이 제공하는 값만 사용할 수 있음
 --> 다른 테이블이 제공하는 컬럼에는 PK또는 U 제약 조건이 설정되어 있어야 한다!
...
--> 50이라는 값은 USER_GRADE 테이블 GRADE_CODE 컬럼에서 제공하는 값이 아니므로
 -- 외래키 제약 조건에 위배되어 오류 발생.
		-- 부모 키가 없습니다
		--> 참조하는 테이블(USER_GRADE)에 값이 존제하지 않음
...
DELETE FROM USER_GRADE 
WHERE GRADE_CODE = 30;
--ORA-02292: 무결성 제약조건(KHK.GRADE_CODE_FK1)이 위배되었습니다- 
--자식 레코드가 발견되었습니다
...
REFERENCES DEPARTMENT; -- DEPARTMENT 테이블 PK컬럼 참조
